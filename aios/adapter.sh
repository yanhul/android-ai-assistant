...
if gradle --no-daemon testDebugUnitTest assembleDebug >/tmp/aios-android-test.log 2>&1; then
  ...
  printf '{"status":"PASS","artifact_refs":["app/build/outputs/apk/debug/app-debug.apk"],"evidence_refs":["android-build-sha256:%s"],"verification_refs":["android_ci","authority_boundary"],"provenance":{"producer":"yanhul/android-ai-assistant","adapter":"android.assistant@1"}}\n' "$sha256"
else
